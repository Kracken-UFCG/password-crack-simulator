...
// Mantemos todas as funções matemáticas da branch pipeline-script
...
double entropy_class_based(const std::string &password) {
    ...
    if (has_symbol) charset += 9;
    return (charset == 0) ? 0.0 : password.size() * log2((double)charset);
}
...
// Cabeçalho unificado com KDF e métricas de Entropia
void print_csv_header() {
    cout << "password,strength,entropy_max,entropy_class,entropy_shannon,search_space,early_ratio,leaked,"
...
// Assinatura da função usando KdfMeta (necessário para sua branch)
                   ...
                   const BruteResult& brute)
{
    ...
    if (dict.leaked) {
        ...
        latency_ns = (attempts > 0) ? (dict.elapsed_sec * 1e9 / (double)attempts) : 0.0;
    } else if (brute.found) {
        method     = "brute_force";
    ...
    // Cálculos de métricas vindos da pipeline
    int len = password.size();
    double ent_max = entropy_max(len);
    double ent_class = entropy_class_based(password);
    double ent_shannon = entropy_shannon(password);
    uint64_t search_space = (uint64_t)compute_search_space(len);
    double early_ratio = (search_space > 0) ? (double)attempts / (double)search_space : 0.0;
    double attempts_per_sec = (cracktime > 0.0) ? (double)attempts / cracktime : 0.0;

    cout << fixed << setprecision(9);

    // Saída final respeitando a ordem do novo cabeçalho
    cout << password
         << "," << strength
         << "," << ent_max
         << "," << ent_class
